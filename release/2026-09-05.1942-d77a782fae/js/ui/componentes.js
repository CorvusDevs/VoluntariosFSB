...
export function manejarTecladoDialogo(evento, dialogo, alCerrar = null) {
  ...
  const controles = [...dialogo.querySelectorAll('a[href], button, input, select, textarea, summary, [contenteditable="true"], [tabindex]')]
    .filter((control) => !control.disabled && !control.hidden && control.getAttribute('aria-hidden') !== 'true' && control.tabIndex >= 0)
  if (!controles.length) { evento.preventDefault(); dialogo.focus(); return true }
  const primero = controles[0]
  const ultimo = controles.at(-1)
  if (evento.shiftKey && (document.activeElement === primero || !dialogo.contains(document.activeElement))) {
    evento.preventDefault(); ultimo.focus(); return true
  }
  if (!evento.shiftKey && document.activeElement === ultimo) {
    evento.preventDefault(); primero.focus(); return true
  }
  return false
}
