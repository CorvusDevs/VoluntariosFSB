...
export function crearPantallaPersonas(raiz, { roster, almacen, alCambiar, esAdmin = false }) {
  ...
  function editor(p) {
    ...
    const nacimiento = campoPerfil('anioNacimiento', 'Fecha de nacimiento', 'date'); const hoy = new Date(); nacimiento.max = hoy.toISOString().slice(0, 10); const selectorAnio = document.createElement('select'); selectorAnio.className = 'selector-anio-nacimiento'; selectorAnio.setAttribute('aria-label', 'Año de nacimiento'); selectorAnio.appendChild(new Option('Elegir año', '')); for (let anio = hoy.getFullYear(); anio >= 1900; anio -= 1) selectorAnio.appendChild(new Option(String(anio), String(anio))); const sincronizarAnio = () => { selectorAnio.value = /^\d{4}/.test(nacimiento.value) ? nacimiento.value.slice(0, 4) : '' }; const edad = elemento('p', ['persona-edad']); const actualizarEdad = () => { const valor = edadDesdeAnio(nacimiento.value); edad.textContent = valor === null ? 'Edad: agregar una fecha de nacimiento válida' : `Edad: ${valor} años` }; selectorAnio.addEventListener('change', () => { if (!selectorAnio.value) return; const [, mes = '01', dia = '01'] = nacimiento.value.split('-'); nacimiento.value = `${selectorAnio.value}-${mes}-${dia}`; actualizarEdad() }); nacimiento.addEventListener('input', () => { sincronizarAnio(); actualizarEdad() }); sincronizarAnio(); actualizarEdad(); bio.append(selectorAnio, edad); campoPerfil('desde', 'En la organización desde', 'date'); campoPerfil('leGusta', 'Le gusta', 'textarea', 'Actividades, intereses o motivadores'); campoPerfil('noLeGusta', 'Prefiere evitar', 'textarea', 'Situaciones, sonidos o actividades'); campoPerfil('necesidades', 'Necesidades y apoyos', 'textarea', 'Información útil para acompañar a la persona')
