...
export function crearPantallaPersonas(raiz, { roster, almacen, alCambiar, esAdmin = false }) {
  ...
  function editor(p) {
    ...
    const nacimiento = campoPerfil('anioNacimiento', 'Fecha de nacimiento', 'date'); nacimiento.max = new Date().toISOString().slice(0, 10); const edad = elemento('p', ['persona-edad']); const actualizarEdad = () => { const valor = edadDesdeAnio(nacimiento.value); edad.textContent = valor === null ? 'Edad: agregar una fecha de nacimiento válida' : `Edad: ${valor} años` }; nacimiento.addEventListener('input', actualizarEdad); actualizarEdad(); bio.appendChild(edad); campoPerfil('desde', 'En la organización desde', 'date'); campoPerfil('leGusta', 'Le gusta', 'textarea', 'Actividades, intereses o motivadores'); campoPerfil('noLeGusta', 'Prefiere evitar', 'textarea', 'Situaciones, sonidos o actividades'); campoPerfil('necesidades', 'Necesidades y apoyos', 'textarea', 'Información útil para acompañar a la persona')
    form.appendChild(bio)
    form.appendChild(boton(nueva ? 'Agregar persona' : 'Guardar cambios', async () => { const limpio = nombre.value.trim(); if (!limpio) return; const perfilNuevo = Object.fromEntries([...bio.querySelectorAll('[data-perfil]')].map((e) => [e.dataset.perfil, e.value.trim()])); if (nueva) { const siguiente = clase.value === 'participante' ? agregarParticipante(actual, { nombre: limpio, grupo: Number(selectorGrupo.value), nuevo: marca.checked, notas: notas.value, perfil: perfilNuevo }) : agregarVoluntario(actual, { nombre: limpio, nuevo: marca.checked, notas: notas.value, perfil: perfilNuevo }); agregando = false; await guardar(siguiente, `Agregar a ${limpio}`) } else { const cambios = { nombre: limpio, nuevo: marca.checked, notas: notas.value, perfil: perfilNuevo }, mudanzas = []; if (datos.tipo === 'participante' && Number(selectorGrupo.value) !== datos.grupo) { cambios.grupo = Number(selectorGrupo.value); mudanzas.push({ id: datos.id, grupo: cambios.grupo }) } await guardar(editarPersona(actual, datos.id, cambios), `Actualizar a ${datos.nombre}`, mudanzas) } }))
    form.addEventListener('submit', (e) => e.preventDefault()); caja.appendChild(form)
    if (!nueva) { const vista = elemento('section', ['vista-tarjeta-personal']); vista.appendChild(elemento('h3', [], 'Vista previa de tarjeta')); const textoVista = elemento('p', ['ayuda-ajustes'], 'Así se descargará la tarjeta PNG.'); vista.appendChild(textoVista); lienzoDeTarjeta(datos).then((lienzo) => { if (!vista.isConnected) return; lienzo.className = 'lienzo-tarjeta-personal'; vista.appendChild(lienzo) }).catch(() => { textoVista.textContent = 'No se pudo preparar la vista previa.' }); caja.appendChild(vista) }
    ...
    if (!nueva) { const archivo = document.createElement('input'); archivo.type = 'file'; archivo.accept = 'image/*'; archivo.capture = 'user'; archivo.className = 'oculto-visualmente'; archivo.addEventListener('change', async () => { if (archivo.files?.[0]) await foto(datos, archivo.files[0]) }); const fotoBoton = elemento('label', ['boton', 'boton-foto'], datos.foto ? 'Cambiar foto' : 'Agregar foto'); fotoBoton.appendChild(archivo); acciones.appendChild(fotoBoton)
      acciones.appendChild(boton('Descargar tarjeta PNG', () => descargarTarjeta(datos)))
